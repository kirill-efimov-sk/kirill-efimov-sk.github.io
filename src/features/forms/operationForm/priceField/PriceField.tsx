...
export type PriceFieldProps = Pick<OperationFormProps, 'className' | 'disabled'> & {
  submitCount: number;
  touched: boolean;
  errors: string;
  value: number;
  ...
  ({ className, onChange, onBlur, touched, value, errors, disabled, submitCount }) => {
    ...
    return (
      <FormItem
        ...
        title={t('forms.OperationForm.price.title')}
        required
      ...
      >
        <InputNumber
          ...
          min={1}
          value={value}
          placeholder={t('forms.OperationForm.price.placeholder')}
          style={{ width: 150 }}
        />
      </FormItem>
    );
